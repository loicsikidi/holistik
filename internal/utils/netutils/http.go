...
var (
	ErrHTTPGetTooLarge = ergo.NewSentinel("downloaded content exceeds maximum allowed size")
	ErrHTTPGetError    = ergo.NewSentinel("error during HTTP GET request")
)
...
func HttpGET(ctx context.Context, client HTTPClient, url string, optionalMaxLength ...int64) ([]byte, error) {
	maxLength := utils.OptionalArgWithDefault(optionalMaxLength, DefaultMaxSize)
	c := client
	if c == nil {
	...
	operation := func() ([]byte, error) {
		...
		if res.StatusCode >= 500 && res.StatusCode < 600 {
			...
			return nil, sentinel.Errorf("failed to download from %s: HTTP %d", url, res.StatusCode)
		}

		if res.StatusCode != http.StatusOK {
			res.Body.Close() //nolint:errcheck
			err := sentinel.Errorf("failed to download from %s: HTTP %d", url, res.StatusCode)
			return nil, backoff.Permanent(sentinel.NewAggregate(ErrHTTPGetError, err))
		}
		...
		if header := res.Header.Get("Content-Length"); header != "" {
			...
			if length > maxLength {
				res.Body.Close() //nolint:errcheck
				err := sentinel.Errorf("download failed for %s, length %d is larger than expected %d", url, length, maxLength)
				return nil, backoff.Permanent(sentinel.NewAggregate(ErrHTTPGetTooLarge, err))
			}
		}
	...
	if err != nil {
		...
		for code := 500; code < 600; code++ {
			if strings.Contains(errMsg, "HTTP "+strconv.Itoa(code)) {
				return nil, sentinel.NewAggregate(ErrHTTPGetError, err)
			}
		}
